...
즉, "사람이 평가를 수행하되, 측정과 계산은 시스템이 담당하는 구조"로 전환함으로써 평가 결과의 일관성을 향상시킬 수 있다.

# 14. 참고 프로젝트와의 차별성

참고 프로젝트에서는 건물 특성을 활용한 에너지 소비량 또는 효율 예측에 집중하였다. 반면 본 연구는 EPC 평가 결과 자체를 분석 대상으로 설정하고, 동일 건물에 대한 반복 평가 결과와 머신러닝 기반 예측 결과를 활용하여 평가 체계의 신뢰성과 일관성을 검증하였다. 또한 예측 오차를 단순 성능 지표가 아닌 분석 대상으로 활용하여 Hidden Inefficiency 및 Strong Anomaly 사례를 도출하였다는 점에서 기존 예측 중심 연구와 차별성을 가진다.
